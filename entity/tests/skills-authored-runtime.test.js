...
function createSkillAuthoredPayload(name = 'runtime-skill') {
  return {
    name,
...
function authoredRoot(testName) {
  return join(process.cwd(), 'entity-workspace', 'test-artifacts', testName);
}

async function writeAuthoredSkill({ root, name, code, approved, codeHash }) {
  ...
  const baseSkillSource = await readFile(join(process.cwd(), 'src', 'skills', 'base-skill.js'), 'utf-8');
  await Promise.all([
    writeFile(join(skillPath, 'manifest.json'), JSON.stringify(manifest, null, 2)),
    writeFile(join(skillPath, 'index.js'), code),
    writeFile(join(skillPath, 'base-skill.js'), baseSkillSource),
    writeFile(
...
describe('Authored skill runtime', () => {
  ...
  it('passes sandbox validation path for generated authored skill', async (t) => {
  ...
  it('saveSkill writes base-skill and code hash metadata', async (t) => {
  ...
  it('loads only approved authored skills with valid code hash', async (t) => {

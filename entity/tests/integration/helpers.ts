...
export async function canBindLoopback() {
  return new Promise((resolveCanBind: any) => {
    const server = createServer();
...
export async function createMindFixture(rootDir: any) {
...
function createTestMindServer(mindPath: any) {
  return {
...
function createConfig({ tempRoot, mindPath, httpPort, wsPort, apiKey = null, autonomy = 'balanced' }: any) {
...
function createDeterministicLlmResponder(behavior: any = {}) {
  ...
  const payloadForPhase = (phase: any) => {
    if (phase === 'think') {
      return {
  ...
  return async (_systemPrompt: any, _messages: any, options: any = {}) => {
...
export async function startInProcessEntity(options: any = {}) {
...
export async function getJson(baseUrl: any, path: any, apiKey: any = null) {
...
export async function openWsClient(url: any, options: any = {}) {
  const ws = new WebSocket(url, {
    headers: options.headers || {},
  });
  const messages: any[] = [];

  await new Promise((resolveOpen: any, rejectOpen: any) => {
  ...
  const waitFor = async (predicate: any, timeoutMs: any = 5000) => {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      const found = messages.find(predicate);
      if (found) return found;
      await new Promise((resolveWait: any) => setTimeout(resolveWait, 20));
    }
  ...
  const close = async () => {
    ...
    await new Promise((resolveClose: any) => {
...
export async function waitForCondition(check: any, timeoutMs: any = 5000, intervalMs: any = 20) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    if (check()) return true;
    await new Promise((resolveWait: any) => setTimeout(resolveWait, intervalMs));
  }
  return false;
}

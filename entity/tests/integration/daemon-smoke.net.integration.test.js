...
async function waitForHttpHealth(baseUrl, timeoutMs = 30000) {
...
async function waitForProcessExit(child, timeoutMs = 15000) {
...
describe('Daemon smoke (net-gated)', () => {
  it('boots daemon with mock LLM and handles one HTTP+WS cycle', async (t) => {
    ...
    const cycleComplete = await wsClient.waitFor(
      (message) => message.type === 'cycle_complete' && message.cycleId === data.cycleId,
      10000
    );

...
describe('Approval flow integration', () => {
  it('handles approve and deny round-trips for plan approvals', async (t) => {
    ...
    const approvalNeeded = await wsClient.waitFor(
      (message) => message.type === 'approval_needed' && message.approvalType === 'plan'
    );
    ...
    const approvalResult = await wsClient.waitFor(
      (message) => message.type === 'approval_result' && message.approvalId === approvalNeeded.approvalId
    );
    ...
    const deniedResult = await wsClient.waitFor(
      (message) => message.type === 'approval_result' && message.approvalId === deniedApprovalNeeded.approvalId
    );
  ...
  it('does not emit plan approvals in full_trust mode for tier-3 plans', async (t) => {
    if (!(await canBindLoopback())) {
      t.skip('Loopback sockets are unavailable in this runtime');
      return;
    }
  ...
  it('emits plan approvals for tier-2 plans in conservative mode', async (t) => {
    if (!(await canBindLoopback())) {
      t.skip('Loopback sockets are unavailable in this runtime');
      return;
    }

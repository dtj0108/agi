...
describe('Lifecycle integration', () => {
  it('enforces auth and supports pause/resume/kill lifecycle paths', async (t) => {
    ...
    const unauthorizedClose = await new Promise((resolveClose, rejectClose) => {
      const timeout = setTimeout(() => rejectClose(new Error('Unauthorized WS close timeout')), 5000);
      unauthorizedWs.once('close', (code, reason) => {

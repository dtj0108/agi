...
describe('Skills HTTP Security', () => {
  it('rejects tier-3/4 test calls with approvalRequired and does not execute', async (t) => {
  ...
  it('routes tier-1/2 skill tests through ActionGateway with dashboard source tag', async (t) => {
    if (!(await canBindLoopback())) {
      t.skip('Loopback sockets are unavailable in this runtime');
      return;
    }
  ...
  it('returns skill metadata with authored flag and tier summary', async (t) => {
    if (!(await canBindLoopback())) {
      t.skip('Loopback sockets are unavailable in this runtime');
      return;
    }
    ...
    const actionGateway = {
      engines: {
        skills: {
          getTier: () => 4,
          isAuthored: (name) => name === 'authored-skill',

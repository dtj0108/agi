...
describe('InterfaceLayer lifecycle', () => {
  ...
  it('pause/resume routes through autonomy controller state', () => {
    const cognitiveEngine = new FakeCognitiveEngine();
    const actionGateway = new FakeActionGateway();
    ...
    layer.onPause();
    assert.strictEqual(layer.autonomy.getStatus().paused, true);

    layer.onResume();
    assert.strictEqual(layer.autonomy.getStatus().paused, false);
  });
});

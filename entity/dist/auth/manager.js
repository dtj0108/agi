...
function normalizeTokenSet(tokenResponse, { issuer, clientId, scope }, previous = null) {
    if (!isNonEmptyString(tokenResponse?.access_token)) {
        throw new Error('OAuth token response missing access_token');
    }
    const expiresIn = toNumber(tokenResponse.expires_in, null);
    const computedExpiresAt = expiresIn !== null
...
function isTokenExpired(expiresAtMs, skewMs = 60_000) {
...
function buildDefaultAuthConfig(config) {
    ...
    return {
        ...
        oauth: {
            ...
            extraAuthorizeParams: oauth.extraAuthorizeParams && typeof oauth.extraAuthorizeParams === 'object'
...
export class AuthManager {
    ...
    async loadCredentials({ reload = false } = {}) {
        if (reload) {
            this.credentialsLoaded = false;
        }
    ...
    chooseFlow({ flow, deviceCode = false, noBrowser = false } = {}) {
        if (deviceCode)
            return 'device_code';
    ...
    validateOAuthSettings({ issuer, clientId, scopes }) {
    ...
    async getOAuthAccessToken({ forceRefresh = false } = {}) {
        if (!this.allowsOAuth())
            return null;
    ...
    async getAuthStatus({ reload = true } = {}) {

...
export class SearchIndex {
    ...
    stmts;
    vectorsEnabled;
    constructor(mindPath, dbPath, embeddingService = null) {
     ...
     */
    async indexFile(path, content, generateEmbedding = true) {
     ...
     */
    async rebuildIndex(includeEmbeddings = true) {
        // Clear existing data
    ...
    async initializeVectorTables() {
      ...
      `);
            ...
            this.stmts.getContentId = this.db.prepare('SELECT id FROM mind_files_content WHERE path = ?');
      ...
      `);
            this.stmts.getEmbedding = this.db.prepare('SELECT embedding FROM mind_files_embeddings WHERE content_id = ?');
            this.stmts.deleteEmbedding = this.db.prepare('DELETE FROM mind_files_embeddings WHERE content_id = ?');
     ...
     */
    async indexEmbedding(relativePath, content) {
        if (!this.hasEmbeddings())
            return null;
        try {
            // Get content ID
            ...
            // Store embedding
            this.stmts.upsertEmbedding.run(contentRow.id, embeddingBlob, this.embeddingService.getModel(), new Date().toISOString());
    ...
    async hybridSearch(query, options = {}) {
        ...
        if (vectorResults.results.length === 0) {
            return {
                results: ftsResults.results.map((r) => ({
                    ...r,
        ...
        if (ftsResults.results.length > 0) {
            const maxRank = Math.max(...ftsResults.results.map((r) => Math.abs(r.rank))) || 1;

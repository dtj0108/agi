...
export class UpdatePhase {
     ...
     */
    async execute(context, thought, reflection, observations) {
        const filesUpdated = [];
        const timestamp = new Date().toISOString();
        // 1. Update emotional state
        const newEmotionalState = this.updateEmotionalState(context.emotions, thought.emotionalShift, reflection?.emotionalUpdate);
        await this.saveEmotionalState(newEmotionalState);
        filesUpdated.push('emotions/state.json');
        // 2. Create episodic memory
        const memoryPath = await this.createEpisodicMemory(timestamp, thought, reflection, observations);
     ...
     */
    updateEmotionalState(currentState, thinkShift, reflectUpdate) {
        // Combine shifts from think and reflect phases
        const primaryDelta = ((thinkShift?.primary?.delta || 0) +
            (reflectUpdate?.primary?.delta || 0)) / 2;
        ...
        const secondaryDelta = ((thinkShift?.secondary?.delta || 0) +
            (reflectUpdate?.secondary?.delta || 0)) / 2;
     ...
     */
    calculateMomentum(oldIntensity, newIntensity) {
     ...
     */
    calculateInfluences(emotion, intensity) {
        const base = {
     ...
     */
    async createEpisodicMemory(timestamp, thought, reflection, observations) {
...
${observations?.length > 0
            ? observations.map((o) => `- **${o.tool}**: ${o.success ? 'Success' : 'Failed'} - ${o.intent || 'No intent specified'}`).join('\n')
...
`;
     ...
     */
    async updateCapabilities(skill) {
...
${skill.description}`;
     ...
     */
    async updateWorldContext(observations) {
        ...
        try {
            ...
            if (observations?.length > 0) {
                const summary = observations
                    ...
                    .join(', ');
                if (summary) {
                    content = content.replace(/## Situation[\s\S]*?(?=##|$)/, `## Situation
- Last cycle: ${now}
...
`);
     ...
     */
    async updatePreferences(observations) {
        const prefsPath = join(this.mindPath, 'self/preferences.md');
        const configObs = observations.filter((o) => o.tool === 'config' && o.success);
        try {
            let content = await readFile(prefsPath, 'utf-8');
            const now = new Date().toISOString();
            const setLine = (pattern, replacement) => {
            ...
            for (const obs of configObs) {
                ...
                if (setting === 'heartbeat.schedule' && result.schedule) {
                    setLine(/(?:-\s*)?\*\*Interval\*\*:.*$/m, `- **Interval**: Every custom schedule (${result.schedule})`);
                    setLine(/(?:-\s*)?\*\*Last changed\*\*:.*$/m, `- **Last changed**: ${now}`);
                }
                ...
                if (setting === 'cognitive.temperature' && result.temperature !== undefined) {
                    setLine(/(?:-\s*)?\*\*Temperature\*\*:.*$/m, `- **Temperature**: ${result.temperature}`);
                }
                if (setting === 'cognitive.emotionalDecayRate' && result.emotionalDecayRate !== undefined) {
                    setLine(/(?:-\s*)?\*\*Emotional Decay Rate\*\*:.*$/m, `- **Emotional Decay Rate**: ${result.emotionalDecayRate}`);
                }
                if (setting === 'cognitive.emotionalMomentum' && result.emotionalMomentum !== undefined) {
                    setLine(/(?:-\s*)?\*\*Emotional Momentum\*\*:.*$/m, `- **Emotional Momentum**: ${result.emotionalMomentum}`);
                }
                if (setting === 'cognitive.reflectionInterval' && result.reflectionInterval !== undefined) {
                    setLine(/(?:-\s*)?\*\*Reflection Interval\*\*:.*$/m, `- **Reflection Interval**: ${result.reflectionInterval} cycles`);
                }
                if (setting === 'cognitive.maxTokens' && result.maxTokens !== undefined) {
                    setLine(/(?:-\s*)?\*\*Max Tokens\*\*:.*$/m, `- **Max Tokens**: ${result.maxTokens}`);
                }
                if (setting === 'cognitive.retryAttempts' && result.retryAttempts !== undefined) {
                    setLine(/(?:-\s*)?\*\*Retry Attempts\*\*:.*$/m, `- **Retry Attempts**: ${result.retryAttempts}`);
                }
            }

 ...
 */
import { readFile, writeFile, appendFile, rename, unlink, readdir, stat, mkdir, } from 'fs/promises';
...
export class FileOperations {
    ...
    constructor(config) {
        ...
        this.allowedPaths = [
            this.workspacePath,
            this.mindPath,
            ...(config.actions?.files?.allowedPaths || []).map((p) => resolve(p)),
        ];
        // Blocked paths
        this.blockedPaths = [
            '/etc',
            ...
            ...(config.actions?.files?.blockedPaths || []).map((p) => resolve(p)),
        ];
    }
    /**

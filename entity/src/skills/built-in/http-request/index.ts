...
export default class HttpRequestSkill extends BaseSkill {
  ...
  validateUrl(urlString: any) {
    ...
    if (!allowedDomains.includes('*')) {
      const isAllowed = allowedDomains.some(
        (allowed: any) => domain === allowed || domain.endsWith(`.${allowed}`)
      );
      if (!isAllowed) {
  ...
  isPrivateTarget(hostname: any) {
    ...
    if (ipv4Match) {
      const parts = normalized.split('.').map((value: any) => Number(value));
      if (parts.some((value: any) => !Number.isInteger(value) || value < 0 || value > 255)) {
        return true;
      }
   ...
   */
  async post({ url, body, headers = {} }: any) {
   ...
   */
  async request({ url, method, body, headers = {} }: any) {
    this.validateUrl(url);

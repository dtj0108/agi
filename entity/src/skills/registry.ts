...
export class SkillsRegistry extends EventEmitter {
  ...
  skills: any;
  validator: any;
  constructor(config: any = {}, mindServer: any = null) {
    super();
    this.skills = new Map();
  ...
  setupHotReload() {
    ...
    this.mindServer.on('skill:authored', async (event: any) => {
    ...
    this.mindServer.on('file:changed', async (event: any) => {
   ...
   */
  async loadBuiltIn(builtInPath: any) {
   ...
   */
  async loadUserSkills(userPath: any) {
   ...
   */
  async loadAuthoredSkills(mindPath: any) {
   ...
   */
  async loadAuthoredSkill(skillPath: any) {
   ...
   */
  async reloadAuthoredSkill(name: any, skillPath: any) {
   ...
   */
  list() {
    const builtIn = Array.from(this.skills.values()).map((skill: any) => skill.getMetadata());
    const authored = Array.from(this.authoredSkills.values()).map((skill: any) => ({
      ...skill.getMetadata(),
      _authored: true,
    }));
   ...
   */
  listBuiltIn() {
    return Array.from(this.skills.values()).map((skill: any) => skill.getMetadata());
  }
   ...
   */
  listAuthored() {
    return Array.from(this.authoredSkills.values()).map((skill: any) => ({
      ...skill.getMetadata(),
      _authored: true,
    }));
  }
  ...
  getContextForLLM() {
    ...
    return skills.map((s: any) => {
      const actions = s.actions.map((a: any) =>
        `    - ${a.name}: ${a.description || 'No description'} (Tier ${a.tier || 2})`
      ).join('\n');
  ...
  getAuthoredContextForLLM() {
    ...
    return skills.map((s: any) => {
      const actions = s.actions.map((a: any) =>
        `    - ${a.name}: ${a.description || 'No description'} (Tier 4 - requires approval)`
      ).join('\n');
   ...
   */
  async reloadSkill(name: any, skillPath: any) {

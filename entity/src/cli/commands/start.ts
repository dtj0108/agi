...
export default async function start(args: any, projectRoot: any) {
  const os = platform();
  ...
  const out = await import('fs').then((fs: any) =>
    fs.openSync(join(logsDir, 'stdout.log'), 'a')
  );
  const err = await import('fs').then((fs: any) =>
    fs.openSync(join(logsDir, 'stderr.log'), 'a')
  );

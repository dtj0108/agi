...
export default async function reset(args: any, projectRoot: any) {
  ...
  const answer1 = await new Promise((resolve: any) => {
  ...
  try {
    await import('./stop.js').then((m: any) => m.default([], projectRoot));
  } catch {
    // Ignore
  }

...
export default async function logs(args: any, projectRoot: any) {
  ...
  const entries = content.split('---').filter((e: any) => e.trim());
  ...
  // Watch for changes
  const watcher = watch(streamPath, (eventType: any) => {
    if (eventType === 'change') {

...
function formEncode(payload: any) {
  return new URLSearchParams(
    Object.entries(payload)
      ...
      .map(([key, value]: any) => [key, String(value)] as [string, string])
  ).toString();
}

async function readJsonOrText(response: any) {
...
function createTimeoutSignal(timeoutMs: any) {
...
export class OidcProvider {
  metadataCache: any;
  timeoutMs: any;
  constructor({ timeoutMs = 20000 }: any = {}) {
    this.timeoutMs = timeoutMs;
    this.metadataCache = new Map();
  }

  normalizeIssuer(issuer: any) {
  ...
  async discoverMetadata(issuer: any, { force = false }: any = {}) {
  ...
  async startDeviceAuthorization({ metadata, issuer, clientId, scope }: any) {
    const normalizedIssuer = this.normalizeIssuer(issuer);
  ...
  async pollDeviceToken({ metadata, issuer, clientId, deviceCode, interval = 5, expiresIn = 900 }: any) {
    const normalizedIssuer = this.normalizeIssuer(issuer);
    const tokenEndpoint = metadata?.token_endpoint || `${normalizedIssuer}/oauth/token`;
  ...
  async refreshToken({ metadata, issuer, clientId, refreshToken, scope }: any) {
    const normalizedIssuer = this.normalizeIssuer(issuer);
    const tokenEndpoint = metadata?.token_endpoint || `${normalizedIssuer}/oauth/token`;

...
export class EmbeddingService {
   ...
   */
  async embedBatch(texts: any) {
    if (!this.isEnabled()) {
      throw new Error('Embedding service is not enabled');
    }

    // Filter empty texts
    const validTexts = texts.filter((t: any) => t && t.trim());
   ...
   */
  async embedWithRetry(texts: any) {
    let lastError: any;
  ...
  async embedLocal(texts: any) {
    ...
    try {
      ...
      const data: any = await response.json();
      return data.embeddings.map((emb: any) => new Float32Array(emb));
    } finally {
      clearTimeout(timeoutId);
    }
  }
  ...
  async handleError(response: any) {
    ...
    const error: any = new Error(`Embedding API error: ${status} - ${body}`);

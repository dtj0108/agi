...
export async function loadConfig() {
  ...
  config.actions.files.allowedPaths = config.actions.files.allowedPaths.map((p: any) => resolve(projectRoot, p));
  config.actions.files.blockedPaths = config.actions.files.blockedPaths.map((p: any) => resolve(projectRoot, p));

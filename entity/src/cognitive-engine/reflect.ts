...
export class ReflectPhase {
   ...
   */
  async execute(context: any, thought: any, plan: any, observations: any) {
    // Build the prompt
   ...
   */
  clampValue(value: any, min: any, max: any) {
   ...
   */
  async saveReflection(reflection: any, thought: any, plan: any, observations: any) {
...
${observations?.length > 0
  ? observations.map((o: any) => `- ${o.tool}: ${o.success ? 'Success' : 'Failed'} - ${o.intent}`).join('\n')

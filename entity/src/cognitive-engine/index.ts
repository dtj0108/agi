...
export class CognitiveEngine extends EventEmitter {
  ...
  think: any;
  update: any;
  constructor(config: any, actionGateway: any, mindServer: any, skillsExecutor: any = null) {
    super();
    this.config = config;
    ...
    this.lastThoughts = [];
    this.cycleQueue = Promise.resolve();
    ...
    this.think = new ThinkPhase(this.llm, this.promptBuilder, this.mindPath, {
      fallbackFactory: (context: any, stimulus: any) => this.createThinkFallback(context, stimulus),
    });
    this.plan = new PlanPhase(this.llm, this.promptBuilder, this.mindPath, actionGateway, {
      fallbackFactory: (context: any, thought: any) => this.createPlanFallback(context, thought),
    });
    ...
    this.reflect = new ReflectPhase(this.llm, this.promptBuilder, this.mindPath, {
      fallbackFactory: (context: any, thought: any, plan: any, observations: any) =>
    ...
    this.update = new UpdatePhase(
      ...
      config,
      mindServer,
      (skillApprovalPayload: any) => this.requestSkillAuthorApproval(skillApprovalPayload)
    );

    // Forward act phase events
    this.act.on('approval_needed', (data: any) => this.emit('approval_needed', data));
  }

  /**
   * Route self-authored skill approval through ActionGateway pending approvals.
   */
  async requestSkillAuthorApproval(skillApprovalPayload: any) {
   ...
   */
  async runCycle(stimulus: any) {
    const execute = async () => this.runCycleInternal(stimulus);
    const queued = this.cycleQueue.then(execute, execute);
    this.cycleQueue = queued.catch(() => {});
    return queued;
  }
   ...
   */
  async runCycleInternal(stimulus: any) {
    if (this.paused) {
   ...
   */
  async handlePhaseError(phase: any, error: any, stimulus: any) {
...
I need to continue despite this error. Moving to next phase with defaults.`;
   ...
   */
  async enterSafeMode(phase: any, error: any) {
    this.safeMode = true;
...
Will only respond to direct user messages until issue is resolved.`;
   ...
   */
  async appendToThoughtStream(content: any) {
   ...
   */
  async logError(phase: any, error: any) {
   ...
   * Get recent thoughts
   */
  async getRecentThoughts(limit: any = 5) {
    return this.lastThoughts.slice(-limit).map((content: any, i: any) => ({
      content,
      timestamp: new Date().toISOString(),
    }));
  }
   ...
   */
  async writeThought(content: any) {
  ...
  updateCognitiveConfig(key: any, value: any) {
    const parseNumber = (raw: any, { min, max, integer = false, label }: any) => {

...
export class BrowserController {
  ...
  constructor(config: any) {
    ...
    this.allowedDomains = new Set(
      (config.actions?.browser?.allowedDomains || [])
        .map((domain: any) => String(domain).trim().toLowerCase())
        .filter(Boolean)
    );
  ...
  async getSemanticSnapshot() {
    ...
    const traverse = (node: any, depth: any = 0) => {
      if (!node) return;
   ...
   */
  async findElementByRef(ref: any) {
   ...
   * Scroll the page
   */
  async scroll(direction: any = 'down', amount: any = 300) {
    const deltaY = direction === 'down' ? amount : -amount;

    await this.page.evaluate((dy: any) => {

...
export class ActionGateway extends EventEmitter {
  ...
  async executeAction(action: any) {
    ...
    try {
      switch (action.tool) {
        case 'shell':
          result = await this.engines.shell.execute(
   ...
   */
  async requestApproval(actionId: any, action: any) {
   ...
   */
  getApprovalThreshold(level: any = this.getAutonomyLevel()) {
    switch (level) {
      case 'conservative':
        return 2;
      case 'balanced':
   ...
   */
  requiresApprovalForTier(tier: any) {
   ...
   */
  getPendingApprovals() {
    return Array.from(this.pendingApprovals.entries()).map(([id, pending]: any) => ({
      id,
   ...
   */
  async executeConfig(params: any) {

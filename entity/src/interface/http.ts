...
export class HttpServer extends EventEmitter {
  ...
  paused: any;
  server: any;
  constructor(config: any, cognitiveEngine: any, actionGateway: any, mindServer: any, autonomy: any = null, authManager: any = null) {
    super();
    this.config = config;
  ...
  setupRoutes() {
    ...
    this.app.get('/auth/status', async (req: any, res: any) => {
      try {
        const auth = await this.authManager.getAuthStatus();
        res.json(auth);
      } catch (err: any) {
    ...
    this.app.get('/diagnostics/errors', async (req: any, res: any) => {
      try {
    ...
    this.app.get('/diagnostics/metrics', async (req: any, res: any) => {
      try {
    ...
    // Read a mind file
    this.app.get(/^\/mind\/(.+)/, async (req: any, res: any) => {
      try {
    ...
    this.app.post('/approve/:actionId', (req: any, res: any) => {
    ...
    this.app.post('/deny/:actionId', (req: any, res: any) => {
      const { actionId } = req.params;
    ...
    // Hard kill
    this.app.post('/kill', (req: any, res: any) => {
      res.json({ message: 'Shutting down...' });
      this.emit('kill');
    });

    // Soft pause
    this.app.post('/pause', (req: any, res: any) => {
      this.paused = true;
    ...
    // Resume
    this.app.post('/resume', (req: any, res: any) => {
      this.paused = false;
    ...
    this.app.post('/go', (req: any, res: any) => {
      try {
        if (!this.autonomy) {
    ...
    this.app.post('/stop', (req: any, res: any) => {
      try {
        if (!this.autonomy) {
          return res.status(503).json({ error: 'Autonomy controller unavailable' });
        }
    ...
    this.app.post('/rollback', async (req: any, res: any) => {
      try {
    ...
    // Get action history
    this.app.get('/history', async (req: any, res: any) => {
      try {
    ...
    this.app.get('/approvals', async (req: any, res: any) => {
      try {
    ...
    // Get configuration
    this.app.get('/config', (req: any, res: any) => {
    ...
    this.app.put('/config', (req: any, res: any) => {
      try {
        ...
        if (blockedPatternsCandidate !== undefined) {
          if (!Array.isArray(blockedPatternsCandidate) || blockedPatternsCandidate.some((p: any) => typeof p !== 'string')) {
            return res.status(400).json({
              success: false,
    ...
    this.app.post('/cycle', async (req: any, res: any) => {
      if (this.paused) {
        return res.status(503).json({ error: 'Entity is paused' });
      }
      try {
   ...
   */
  setupSkillsRoutes(telemetry: any) {
    // Get all skills
    this.app.get('/skills', (req: any, res: any) => {
      try {
        ...
        const skills = skillsExecutor
          .listSkills()
          .map((skill: any) => this.withSkillMetadata(skill, skill?._authored === true));
        res.json({ skills });
      } catch (err: any) {
    ...
    this.app.get('/skills/:name', (req: any, res: any) => {
      try {
    ...
    // Test a skill action
    this.app.post('/skills/:name/test', async (req: any, res: any) => {
      try {
        const { name } = req.params;
   ...
   */
  withSkillMetadata(skill: any, authored: any = false) {
    const actions = Array.isArray(skill?.actions) ? skill.actions : [];
    const tiers = actions
      .map((action: any) => Number(action?.tier))
      .filter((tier: any) => Number.isFinite(tier));
    ...
    return {
      ...
      tierSummary: {
        ...
        minTier,
        maxTier,
        requiresApproval: tiers.some((tier: any) => tier >= 3),
      },
    };
  }
...
npm run build</pre>
   ...
   */
  validateAutonomyLevel(value: any) {
   ...
   */
  validateAutonomyMode(value: any) {

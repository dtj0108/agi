...
export function Mind() {
  ...
  async function loadFile(path: string) {
    try {
      ...
      setFileContent({ error: getErrorMessage(err) })
    }
  }
  ...
  function renderTree(items: Record<string, any> | null, basePath = "") {
    if (!items) return null

    return (
      <div className="space-y-1">
        {Object.entries(items).map(([name, rawValue]) => {
          const value = rawValue as any
          const path = basePath ? `${basePath}/${name}` : name
          const isFolder =
            typeof value === "object" && value !== null && value.content === undefined
            ...
            return (
              <div key={path}>
                ...
                >
                  ...
                  {isExpanded && (
                  <div className="ml-4">{renderTree(value as Record<string, any>, path)}</div>
                )}
              </div>
            )
          }
...
function FileContentViewer({ content, path }: { content: any; path: string | null }) {
  if (content.error) {
    return (
...
function EmotionViewer({ emotion }: { emotion: any }) {
  return (
      ...
      <div>
        ...
        {emotion.triggers?.length > 0 ? (
          <ul className="space-y-2">
            {emotion.triggers.map((trigger: string, i: number) => (
...
function GoalsViewer({ goals }: { goals: any[] }) {
  return (

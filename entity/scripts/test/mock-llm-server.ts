...
function detectPhase(promptText: any = '') {
...
function buildPayload(phase: any, behavior: any = {}) {
...
function buildPromptTextFromOpenAI(body: any = {}) {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  return messages.map((message: any) => contentToText(message.content)).join('\n');
}

function buildPromptTextFromAnthropic(body: any = {}) {
  const system = Array.isArray(body.system)
    ? body.system.map((block: any) => contentToText(block)).join('\n')
    : contentToText(body.system);

  const messages = Array.isArray(body.messages)
    ? body.messages.map((message: any) => contentToText(message.content)).join('\n')
    : '';

  return `${system}\n${messages}`;
}

export async function startMockLlmServer(options: any = {}) {
  ...
  app.post('/chat/completions', (req: any, res: any) => {
  ...
  const server = await new Promise((resolveServer: any) => {

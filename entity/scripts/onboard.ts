 ...
 * Usage:
 *   npm run onboard
 *   tsx scripts/onboard.ts
 *
 * Non-interactive mode:
 *   tsx scripts/onboard.ts --non-interactive \
 *     --user-name "Drew" \
 *     --entity-name "Atlas" \
 ...
 * Phase 1: Welcome
 */
async function phase1Welcome(interactive: any) {
 ...
 */
async function phase2MeetUser(args: any) {
  if (!args.interactive) {
    return {
 ...
 */
async function phase3NameEntity(args: any) {
  if (!args.interactive) {
    return {
 ...
 * Phase 4: LLM Setup
 */
async function phase4LLMSetup(args: any) {
  if (!args.interactive) {
  ...
  switch (provider) {
    ...
    case 'openai': {
      const authMethod = await p.select({
        message: 'How should Entity authenticate to OpenAI?',
        options: [
          { value: 'api_key', label: 'API key', hint: 'fastest setup' },
          { value: 'oauth', label: 'Sign in (OAuth)', hint: 'local browser/device-code login' },
        ],
      ...
      if (authMethod === 'oauth') {
        ...
        const scopesInput = await p.text({
          message: 'Scopes (space or comma separated):',
          placeholder: 'openid profile email offline_access',
          defaultValue: 'openid profile email offline_access',
        });
        if (p.isCancel(scopesInput)) { p.cancel('Setup cancelled.'); process.exit(0); }
  ...
  if (!config.apiKey) {
    p.log.warn('Skipping live LLM test because no API key is configured.');
  } else {
    // Test connection
 ...
 * Phase 5: Deployment
 */
async function phase5Deployment(args: any) {
  if (!args.interactive) {
    return {
  ...
  if (deployType === 'local') {
    ...
    const daemonAnswer = await p.confirm({
      message: `Install as a background service on ${osName} that starts on boot?`,
      initialValue: true,
    });
    if (p.isCancel(daemonAnswer)) { p.cancel('Setup cancelled.'); process.exit(0); }
    installDaemon = daemonAnswer === true;
  }
 ...
 */
async function phase6InitMind(userProfile: any, entityProfile: any, llmConfig: any, deployConfig: any, interactive: any) {
  if (interactive) {
 ...
 * Phase 7: First Boot
 */
async function phase7FirstBoot(entityProfile: any, deployConfig: any, llmConfig: any, interactive: any) {
  if (interactive) {
    p.log.step(`Waking up ${entityProfile.name}...`);
  }

  await ensureDistBuild(interactive);
  ...
  if (deployConfig.type === 'manual' || (deployConfig.type === 'local' && !deployConfig.installDaemon)) {
    if (interactive) {
      p.note("Run 'npm start' for dist runtime, or 'npm run start:dev' for source runtime.", 'Start Command');
    }
  }

  if (llmConfig?.oauthLogin) {
    if (interactive) {
      p.log.step('Running OAuth sign-in now...');
    }

    const loginArgs = [
      join(PROJECT_ROOT, 'dist', 'cli', 'index.js'),
      'login',
      '--issuer',
      llmConfig.auth?.oauth?.issuer || '',
      '--client-id',
      llmConfig.auth?.oauth?.clientId || '',
    ].filter(Boolean);

    const loginCode = await new Promise((resolve: any) => {
      const proc = spawn(process.execPath, loginArgs, {
        cwd: PROJECT_ROOT,
        stdio: 'inherit',
      });
    ...
    if (loginCode !== 0 && interactive) {
      p.log.warn('OAuth sign-in did not complete during onboarding.');
      p.note(
        `Run this later:\nentity login --issuer \"${llmConfig.auth?.oauth?.issuer || ''}\" --client-id \"${llmConfig.auth?.oauth?.clientId || ''}\"`,
        'Finish OAuth Setup'
      );
    }
  }
...
async function ensureDistBuild(interactive: any) {
  const distIndex = join(PROJECT_ROOT, 'dist', 'index.js');
  const distCli = join(PROJECT_ROOT, 'dist', 'cli', 'index.js');
  if (existsSync(distIndex) && existsSync(distCli)) return;

  if (interactive) {
    p.log.step('Building dist runtime...');
  }

  const npmBin = process.platform === 'win32' ? 'npm.cmd' : 'npm';
  const buildCode = await new Promise((resolve: any) => {
  ...
  if (buildCode !== 0) {
    throw new Error('Failed to build dist runtime. Run `npm run build` and retry.');
  }
}
...
async function main() {
  ...
  // Phase 7: First Boot
  await phase7FirstBoot(entityProfile, deployConfig, llmConfig, args.interactive);
}

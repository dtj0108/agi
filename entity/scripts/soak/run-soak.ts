...
function parseDuration(value: any, fallbackMs: any) {
...
async function getFreePort() {
  return new Promise((resolvePort: any, rejectPort: any) => {
...
async function sleep(ms: any) {
  await new Promise((resolveSleep: any) => setTimeout(resolveSleep, ms));
}

async function waitForHttpHealth(baseUrl: any, timeoutMs: any) {
...
function thresholdProfile(name: any) {
...
async function createMindFixture(rootDir: any) {
...
async function closeWebSocket(ws: any) {
  if (!ws) return;
...
function formatReportMarkdown(report: any) {
...
async function main() {
  ...
  const daemonStdout: any[] = [];
  const daemonStderr: any[] = [];
  ...
  try {
    ...
    daemon.on('exit', (code: any, signal: any) => {
    ...
    try {
      if (daemon && daemon.exitCode === null && daemon.signalCode === null) {
        ...
        const exited = await new Promise((resolveExit: any) => {
